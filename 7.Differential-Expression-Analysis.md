...
本来我们是可以用DESeq2为htseq-count专门提供的 _DESeqDataSetFromHTSeq_ ，然而很尴尬数据不够要自己凑数，所以只能改用 _DESeqDataSetFromMatrix_ 了 :cold_sweat:
...
还可以进一步通过quasi-likelihood (QL)拓展NB模型，用于解释生物学和技术性导致的基因特异性变异 (Lund et al. 2012; Lun, Chen, and Smyth 2016).
```r
